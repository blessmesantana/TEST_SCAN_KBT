...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (scanButton) {
        scanButton.addEventListener('click', () => {
            ...
            // Исправление: сброс overlay и скрытие ручного ввода
            if (qrResultOverlay) {
            ...
            if (videoElement) videoElement.style.display = '';
            startQrScanner();
...
// Логика удаления курьера теперь реализуется в archive-modal-content
    ...
    async function startQrScanner(cameraIdOverride) {
        try {
            ...
            // Расширенный вывод ошибки и состояния переменных
            let errorDetails = '';
            if (err) {
                errorDetails = `name: ${err.name || ''}\nmessage: ${err.message || ''}\nconstraint: ${err.constraint || ''}`;
                if (err.stack) errorDetails += `\nstack: ${err.stack}`;
                errorDetails += `\nobject: ${JSON.stringify(err)}`;
            }
            // Диагностика переменных
            let diagVars = '';
            try {
                diagVars = `cameraIdToUse: ${typeof cameraIdToUse !== 'undefined' ? cameraIdToUse : 'undefined'}\n` +
                    `videoElement: ${videoElement ? 'OK' : 'undefined'}\n` +
                    `stream: ${stream ? 'OK' : 'undefined'}\n` +
                    `availableCameras: ${Array.isArray(availableCameras) ? availableCameras.length : 'undefined'}\n` +
                    `selectedCameraId: ${typeof selectedCameraId !== 'undefined' ? selectedCameraId : 'undefined'}`;
            } catch (e) {
                diagVars = 'Ошибка диагностики переменных';
            }
            console.error('Ошибка камеры:', err, '\nДиагностика:', diagVars);
            showMessage('error', `Ошибка камеры: ${err && err.message ? err.message : err} \n${errorDetails}\n${diagVars}`, '', '', errorDetails + '\n' + diagVars);
            showAllQrIcons();
            if (qrSpinner) qrSpinner.classList.remove('active');
        } finally {
